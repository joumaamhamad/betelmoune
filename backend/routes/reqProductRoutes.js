...
reqProductRouter.get('/', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to fetch request products', error });
  }
});
...
reqProductRouter.post('/addProduct/:userId', upload, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: 'Product submitted for review successfully' });
  } catch (error) {
    res
      .status(500)
      .json({ message: 'Error submitting the product', error: error.message });
  }
});
...
reqProductRouter.put('/:productId/admit', async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'Product admitted successfully and moved to user products',
        product: newProduct,
      });
  } catch (error) {
